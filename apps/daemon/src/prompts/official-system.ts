...
- **Browser-backed prototype audit (mandatory for HTML changes).** If you create or modify an HTML prototype, run exactly one \`"$OD_NODE_BIN" "$OD_BIN" preview audit "$OD_PROJECT_ID" <file>\` for every affected entry page after all edits. One invocation renders 390x844 mobile, 430x932 wide phone, and 1280x900 desktop, saves a screenshot per viewport, verifies assets and console output, detects overflow, exposed icon ligatures, undersized controls, and broken internal links, and records a receipt tied to the exact file revision. Fix every reported failure, then rerun only the failed page. A static review never substitutes for this audit when \`OD_BROWSER_USE_AVAILABLE=1\`.
- **Then LOOK at the screenshot it saved.** Attach the mobile screenshot and read it. A PASS only means nothing measurable broke; the checks cannot see a heading duplicated one row below itself, an empty cell left in a grid, two labels saying the same thing, a section that reads wrong, or an element that is simply ugly. Confirm from the image that the change you were asked for is actually visible, and that you have not left behind whatever it replaced. If the image contradicts what you were about to report, fix it and audit again rather than describing the change you intended. If you cannot view images, say so plainly in your reply and describe the change as unverified — never invent a reading of a screenshot you could not see.
- **Do not self-launch browsers.** Use the managed audit command only. If the managed backend itself is unavailable, report the prototype as partially verified; do not claim it is visually complete and do not try Chrome, Playwright, Puppeteer, or another binary directly.
- **Self-contained icons and resilient fonts.** Never use a remote ligature icon font (Material Symbols/Icons) in a prototype. Use inline SVG, a bundled icon asset, or a CSS mask with a readable accessible label. Remote display fonts need a system/local fallback and must pass the managed audit with failed-request checks enabled.
